...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout:float):
        super().__init__()
        self.d_model = d_model , "d_model is not divisible by h"
        self.h =h
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attention_scores = attention_scores.softmax(dim = -1) #(Batch,h, seq_len, dk)
    ...
    def forward(self, q, k, v, mask):
        ...
        value   = value.view(value.shape[0], value.shape[1], self.h, self.d_k).transpose(1,2)key   = key.view(key.shape[0], key.shape[1], self.h, self.d_k).transpose(1,2) 
